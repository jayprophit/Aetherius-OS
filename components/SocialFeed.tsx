    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 h-full flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
            ...
            {viewAllLink && onSetView && (
                <button onClick={() => onSetView(viewAllLink)} className="text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-blue-500">
                    VIEW ALL
                </button>
            )}
        </div>
    ...
    return (
        <DashboardWidget title="Recommended For You" viewAllLink="creatorMarketplace" onSetView={onSetView}>
            <div className="space-y-3">
                {products.map(item => (
                    <div key={item.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700/50">
        ...
        <DashboardWidget title="Continue Learning" viewAllLink="myLearning" onSetView={onSetView}>
            <div className="space-y-4">
                {userCourses.map(course => (
                    <div key={course.id} className="cursor-pointer" onClick={() => onSetView('courseDetail', { courseId: course.id })}>
        ...
        <DashboardWidget title="Job Recommendations" viewAllLink="jobSearch" onSetView={onSetView}>
            <div className="space-y-3">
                {recommendedJobs.map(job => (
                    <div key={job.id} className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700/50">
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 flex justify-between items-center gap-4">
            <div className="flex-1">
                ...
                <p className="text-gray-600 dark:text-gray-400 mt-1">Connect, create, and stay updated. <a href="#" className="text-blue-500 font-bold hover:underline">Explore community features!</a></p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                <div className="flex space-x-1">
                    <button className={ICON_BUTTON_CLASSES} title="Add Photo"><CameraIcon className="w-5 h-5" /></button>
                    <button className={ICON_BUTTON_CLASSES} title="Add Video"><VideoIcon className="w-5 h-5" /></button>
                    <button className={ICON_BUTTON_CLASSES} title="Add GIF"><GifIcon className="w-5 h-5" /></button>
                    <button className={ICON_BUTTON_CLASSES} title="Add Link"><LinkIcon className="w-5 h-5" /></button>
                    <button className={ICON_BUTTON_CLASSES} title="Add Location"><MapIcon className="w-5 h-5" /></button>
                </div>
                <button className="px-6 py-2 font-semibold text-sm text-white bg-gray-800 dark:bg-blue-600 rounded-md hover:bg-gray-700 dark:hover:bg-blue-500">Post</button>
            </div>
            <div className="flex flex-wrap gap-2 pt-4 mt-4 border-t border-gray-100 dark:border-gray-700">
                {filters.map(filter => (
                    <button key={filter} className="px-3 py-1 text-xs font-semibold bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600">{filter}</button>
                ))}
            </div>
        </div>
    );
                ...
                {peopleYouMayKnowData.map(person => (
                    <div key={person.name} className="flex-shrink-0 w-48 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm overflow-hidden">
                        ...
                        <div className="p-2">
                            ...
                            <button className="w-full mt-2 flex items-center justify-center gap-1.5 bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 font-semibold p-1.5 rounded-md hover:bg-blue-200 dark:hover:bg-blue-900">
...
export const PostCard: React.FC<{ post: Post }> = ({ post }) => (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                <div className="mt-4 rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
                    ...
                    {post.media.type === 'video' && (
                        <div className="relative bg-black">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                <button className="w-16 h-16 bg-black/50 rounded-full flex items-center justify-center text-white hover:bg-black/70 transition-colors">
                    ...
                    {post.media.type === 'file' && (
                        <div className="bg-gray-50 dark:bg-gray-900 p-3 flex justify-between items-center">
                            ...
                            <button className={ICON_BUTTON_CLASSES} title="Download"><ArrowDownTrayIcon className="w-5 h-5"/></button>
                        </div>
                    )}
                </div>
            )}
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 flex">
            <button className="flex-1 flex items-center justify-center gap-2 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors" title="Love"><HeartIcon className="w-5 h-5"/> Like</button>
            <button className="flex-1 flex items-center justify-center gap-2 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors border-l border-gray-200 dark:border-gray-700" title="Comment"><ChatBubbleOvalLeftEllipsisIcon className="w-5 h-5"/> Comment</button>
             <button className="flex-1 flex items-center justify-center gap-2 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors border-l border-gray-200 dark:border-gray-700" title="Share"><ShareIcon className="w-5 h-5"/> Share</button>
        </div>
        ...
        {post.comments.length > 0 && (
         ...
         <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex items-center space-x-3">
             ...
             <input type="text" placeholder="Write a comment..." className="w-full bg-gray-100 dark:bg-gray-700 border-gray-200 dark:border-gray-600 rounded-full h-8 px-4 text-sm focus:ring-blue-500 focus:border-blue-500"/>
         </div>
    </div>
);
        ...
        <div className="space-y-4">
            {blogs.map(blog => (
                <div key={blog.id} className="flex items-center space-x-3">
                    ...
                    <div>
                        <a href="#" className="font-semibold text-sm text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 leading-tight">{blog.title}</a>
...
export const SponsoredWidget: React.FC = () => (
    <div>
        ...
        {sponsoredDataFB.map(item => (
             <a href="#" key={item.title} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700">
    ...
    <Widget title="Latest Updates" seeAll noPadding>
        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
            {latestUpdates.map(update => (
                 <li key={update.id} className="p-4 flex space-x-3 hover:bg-gray-50 dark:hover:bg-gray-700/50">
        ...
        <div className="flex justify-between items-center mb-2 px-4">
            <h3 className="font-semibold text-gray-600 dark:text-gray-400">Contacts</h3>
             <div className="flex items-center gap-2">
                <button className="p-1.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"><SearchIcon className="w-4 h-4"/></button>
                <button className="p-1.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"><EllipsisHorizontalIcon className="w-4 h-4"/></button>
             </div>
        </div>
        {contactsDataFB.map(contact => (
            <a href="#" key={contact.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700">
             ...
             <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                {groups.map(group => (
                    <li key={group.id} className="p-4 flex space-x-3 items-center hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        ...
                        <div className="flex-1">
                            <a href="#" className="font-semibold text-sm text-gray-800 dark:text-gray-100 hover:text-blue-500">{group.name}</a>
