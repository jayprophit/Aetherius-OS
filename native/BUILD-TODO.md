...
CURRENT_ACTIVE_TODO = P4-ALLOSTASIS DONE (Genesis ea0bb8d local; loop tests PASS); NEXT = P4 drive/goal/plan scoping
NEXT_EXECUTABLE_TODO = P4-DRIVE-SCOPE: OBSERVE whether minimal Drive/Goal types can ride existing capability/world modules or need new headers (plan stays last; action stays missing by design)
LAST_VERIFIED_COMMIT = Genesis ea0bb8d (allostasis trend projection in-driver; 5 _na remain) + Aetherius-OS 978906f
LAST_REMOTE_VERIFIED_COMMIT = Aetherius-OS 978906f + IDE d5db272 pushed; Agent-Bridge d7c1277 + Genesis d930d22+bac7aa1+ea0bb8d LOCAL (push blocked: env deny-rule on main)
